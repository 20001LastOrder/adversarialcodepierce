...
def generate_adversarial(model, tokenizer, program):
    ...
    for variable, score in variable_ranking:
        program["source_tokens"] = mask_variable(
            original_tokens, set(variable_locations[variable]), tokenizer.mask_token
        )
        ...
        random_embeddings = torch.nn.Embedding(1, embedding.size(1))
        torch.nn.init.normal_(random_embeddings.weight, mean=0, std=0.02)
        random_mask_embedding = random_embeddings.weight
        embedding[token_locations, :] = random_mask_embedding.to(device)

        error_loc_perturb, localization_candidate_logits = model_forward_embeds(
            model, embedding, tokenized["attention_mask"], tokenized["labels"]
        )
        error_loc_entropy = torch.nn.functional.cross_entropy(
            localization_candidate_logits, error_loc_perturb
        )
