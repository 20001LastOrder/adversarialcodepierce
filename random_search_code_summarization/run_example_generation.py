...
def main():
    ...
    parser.add_argument("--adv_idx", type=int)
    # print arguments
    ...
    eval_example = eval_examples[args.adv_idx]
...
def generate_adversarial(model, tokenizer, example, candidates, args, device):
    ...
    logger.info(original_tokens)
    logger.info(f"variables: {variables}")
    logger.info(f"original prediction: {original_prediction}")
    logger.info(f"original bleu: {original_bleu}")
    logger.info(f"ground truth: {example.target}")
    ...
    for i in range(args.adv_max_iterations):
        ...
        logger.info(f"new bleu: {bleu_score}")
...
def generate_adversarial_random_vector(model, tokenizer, example, args, device):
    ...
    for i in range(args.adv_max_iterations):
        ...
        logger.info(current_state.keys())
        ...
        tokenized_input = tokenizer(new_tokens, max_length=args.max_source_length, truncation=True)
        ...
        for variable, random_vector in current_state.items():
            token_locations = get_token_location(new_word_ids, variable_locations[variable])
