...
class Seq2Seq(nn.Module):
    ...
    * `eos_id`- end of symbol ids in target for beam search.
    ...
    def forward(
        ...
        elif source_embedding is not None:
            outputs = self.encoder(
                inputs_embeds=source_embedding, attention_mask=source_mask
            )
        ...
        if target_ids is not None:
            ...
            zero = (
                ...
                .to(
                    source_ids.device
                    if source_ids is not None
                    else source_embedding.device
                )
            )
            num_examples = (
                source_ids.shape[0]
                if source_ids is not None
                else source_embedding.shape[0]
            )
            for i in range(num_examples):
                ...
                beam = Beam(
                    ...
                    self.eos_id,
                    is_cuda=(
                        source_ids.device.type == "cuda"
                        if source_ids is not None
                        else source_embedding.device.type == "cuda"
                    ),
                )
...
class Beam(object):
    def __init__(self, size, sos, eos, is_cuda=False):
        self.size = size
        self.tt = torch.cuda if is_cuda else torch
